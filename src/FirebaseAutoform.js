...
import { getDatabase, ref, onValue, set, get, child, push, update, connectDatabaseEmulator, serverTimestamp } from "firebase/database";
...
export class FirebaseAutoform extends LitElement {
  ...
  static get properties() {
    return {
      ...
      loginId: { 
        type: String, 
        attribute: 'login-id'  /* Reference id from firebase-loginbutton to listen events */
      },
      emulation: { 
        type: Boolean, 
        attribute: 'emulation', 
  ...
  constructor() {
    ...
    this.isChild = false;
    this.elementSelectedId = null; /* Atributo id del elemento de la bbdd seleccionado para cargar con firebase-autoform */
  ...
  updated(changedProperties) {
    changedProperties.forEach(async (oldValue, propName) => {
      if ((propName === 'elId' || propName === 'path') && this.elementSelectedId !== oldValue && oldValue !== undefined) {
  ...
  _idExits() {
    const included = (this.data.includes) ? this.data.includes(this.elementSelectedId) : Object.keys(this.data).includes(this.elementSelectedId);
  ...
  getDataId(id = this.elementSelectedId) {
  ...
  _createInputMultiple(containerFieldsGroup, labelKey, typeobj) {
    return new Promise(resolve => {
      this._counter[labelKey] = (!this._counter[labelKey]) ? 0 : this._counter[labelKey] += 1;
      ...
      onValue(refDb, (snapshot) => {
        if (snapshot.exists()) {
          ...
          if (valueSnap === null || valueSnap === '') {
            ...
            this._insertModelInNewElement(this.labelsFormatted[labelKey].labelShown);
  ...
  _createFirebaseAutoformChildMultiple(labelKey, containerFieldsGroupBase) {
    return new Promise(async (resolve) => {
      ...
      this.consoleLog('Numero de firebase-autoforms: ', numberFirebaseAutoformElements);
  ...
  _createRichList(labelKey, containerFieldsGroup) {
    return new Promise(resolve => {
      const refDb = ref(this.db, `/model/${  this.labelsFormatted[labelKey].labelShown}`);
  ...
  async save() {
    if (this.elementSelectedId) {
      ...
      await set(ref(this.db, `${this.path  }/${  this.elementSelectedId}`), this.data);
    }
  }
  ...
  _getValueMultiple(el) {
    const paperInputs = Array.from(el.children).filter(_el => _el.tagName === 'PAPER-INPUT');
  ...
  async _saveFirebase() {
    const {data} = this;
    this.elementSelectedId = this.elementSelectedId || await this._getNextId();
    ...
    update(ref(this.db, `${this.path  }/${  this.elementSelectedId}`), data).then(() => {
  ...
  render() {
    ...
    return html`
      ...
      <h3 class='path${_child}'>
        ...
        <span class="id">
          ${this.elementSelectedId && !this.bHideId ? html` ID: ${this.elementSelectedId}` : html``}
        </span>
      ...
      <div class="container">
        <section class="${_child}">
          ...
          ${this.readonly ? html`` : (this.data !== undefined && !this.isChild) ? html`<paper-button id="saveBtn" class="save"
            raised @click="${this.save}">${(this._simple) ? html`Update` : (this.elementSelectedId) ? html`Update [ID ${this.elementSelectedId}]` :
      ...
      ` : html`<div class="waiting">FirebaseAutoform - Waiting for login...</div>`}
    `;
  }
}
